...
export default async function handler(req,res){
 ...
 try {
  ...
  ensure(action!=='import','Impor GoPay sudah diganti dengan pencatatan pemasukan manual. Gunakan tombol Pemasukan dan lampirkan bukti.',410);
  ...
  const attachments={};
  if(['income','expense'].includes(action)&&payload.receipt){const {bytes,type}=validateReceipt(payload.receipt);attachments.receipt=await privateFile('receipts/'+randomUUID(),bytes,type);}
  const result=await updateLedger(ledger=>applyAction(ledger,action,payload,attachments));return res.status(200).json({message:result.message,ledger:viewLedger(result.ledger,role),...(result.selectedMonth?{selectedMonth:result.selectedMonth}:{})});
