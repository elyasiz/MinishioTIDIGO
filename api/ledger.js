...
export default async function handler(req,res){
 ...
 try {
  ...
  const result=await updateLedger(ledger=>applyAction(ledger,action,payload,attachments));return res.status(200).json({message:result.message,ledger:viewLedger(result.ledger,role)});
