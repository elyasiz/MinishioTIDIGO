...
import { categories, validDate, parseAutomaticReport, money } from '../src/ledger.js';
...
export function applyAction(original, action, payload, attachments={}) {
 ...
 if(action==='expense') {
  ...
  let preview;try{preview=parseAutomaticReport(payload.text,ledger.transactions);}catch(e){throw new UserError(e.message);}
  ensure(preview.errors.length===0,'Laporan belum disimpan. '+preview.errors.slice(0,3).map(e=>`Baris ${e.line}: ${e.reason}`).join(' '));
  ...
  const id=randomUUID();
  const batch={id,filename:payload.filename,count:preview.valid.length,total:preview.valid.reduce((s,t)=>s+t.amount,0),createdAt:now,source:attachments.source||null,amountMode:preview.amountMode,duplicates:preview.duplicates.length,excluded:preview.excluded.length};
  ledger.imports.push(batch);ledger.transactions.push(...preview.valid.map(({line,...t})=>({...t,id:randomUUID(),importId:id,createdAt:now})));
  importedMonth=preview.valid.map(t=>t.date.slice(0,7)).sort().at(-1);
  after={id,count:batch.count,total:batch.total};invalidate(preview.valid.map(t=>t.date));message=`${batch.count} transaksi diimpor · ${money(batch.total)}.`+(batch.duplicates?` ${batch.duplicates} duplikat dilewati.`:'')+(batch.excluded?` ${batch.excluded} transaksi gagal/tertunda dilewati.`:'');
