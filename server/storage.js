...
export async function readJson(path){
 // Compressed responses carry a weak W/ ETag that Blob rejects in ifMatch.
 // Read the original representation so the content and strong version agree.
 const blob=await get(path,{access:'private',useCache:false,headers:{'accept-encoding':'identity'}});
 if(!blob)return {value:null,etag:null};
 const value=JSON.parse(await new Response(blob.stream).text());
 const etag=blob.blob.etag;
 if(!etag||etag.startsWith('W/'))throw new UserError('Versi catatan belum dapat diverifikasi. Silakan coba kembali.',503);
 return {value,etag};
}
export async function writeJson(path,value,etag){return put(path,JSON.stringify(value),{access:'private',addRandomSuffix:false,allowOverwrite:!!etag,...(etag?{ifMatch:etag}:{}),contentType:'application/json'});}
