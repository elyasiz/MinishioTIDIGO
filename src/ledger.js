...
export function readTable(text) {
  ...
  const delimiter = ['\t',';','|',','].sort((a,b) => fields(lines[0],b).length - fields(lines[0],a).length)[0];
  const headers = fields(lines[0], delimiter);
  ...
  const rows = lines.slice(1).map((line, index) => { try { return { line: index+2, values: fields(line, delimiter) }; } catch(e) { return {line:index+2, values:[], error:e.message}; } });
  const normalized = headers.map(x => x.toLowerCase().replace(/[^a-z0-9]/g,''));
  const aliases = { id:['id','transactionid','idtransaksi','nomortransaksi','orderid'], date:['tanggal','date','transactiondate','tanggaltransaksi'], amount:['nominal','amount','grossamount','jumlah','total'], status:['status','transactionstatus','statustransaksi'], description:['keterangan','description','deskripsi'], fee:['biaya','fee','mdr','servicefee'] };
  const mapping = Object.fromEntries(Object.entries(aliases).map(([key,list]) => [key, String(normalized.findIndex(h => list.includes(h)))]));
...
export function parseReport(text, mapping, existing = [], amountMode = 'gross') {
  ...
  for (const row of table.rows) {
    try {
      ...
      const transaction = { reference, date, amount, gross:amountMode==='gross'?raw:(fee === null ? null : raw+fee), fee, description:'Penjualan QRIS', type:'income', category:'Penjualan QRIS', source:'GoPay', line:row.line };
